...
<!-- 
### Getting Started with Campsite
To get started with Campsite, follow these steps:
1. **Install Node.js**: Ensure you have Node.js version 14 or higher installed on your machine. You can download it from [nodejs.org](https://nodejs.org/).
2. **Install Campsite**: Use npm or yarn to install Campsite globally.
   ```bash
   npm install -g campsitejs
   ```
3. **Create a New Site**: Run the following command to create a new Campsite project.
   ```bash
   campsitejs my-site-name
   ```
4. **Navigate to Your Site**: Change into your new site directory.
   ```bash
   cd my-site-name
   ```
5. **Start the Development Server**: Launch the development server to preview your site.
   ```bash
   campsitejs dev
   ```
6. **Build for Production**: When you're ready to deploy, build your site with:
   ```bash
   campsitejs build
   ```
   The output will be in the `./campfire/` folder, which can be changed in your `campsite.config.js` file. -->
